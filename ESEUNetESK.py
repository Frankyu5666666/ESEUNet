...
class Hoblock(nn.Module):
    def __init__(self, in_channels, out_channels, ratio, down=False):
        super().__init__()
        ...
        self.conv1 = SKConvnblockplus(in_channels // 2, out_channels // 2)
        self.conv2 = SKConvnblockplus(in_channels // 2, out_channels // 2)
        self.down = down
...
class HoEncoder(nn.Module):
    def __init__(self, in_channels, out_channels, ratio, shuffle=True):
        ...
        for i in range(1, len(self.channels)):
            setattr(self, f"c{i}", Hoblock(self.channels[i-1], self.channels[i], ratio, down=True))
...
class HoDecoder(nn.Module):
    def __init__(self, in_channels, depth):
        ...
        for i in range(1, depth+1):
            ...
            setattr(self, f"c{i}",
                    SKConvnblockplus(in_channels, in_channels // 2))
        self.up = Up()
...
if __name__ == '__main__':
    ...
    model = ESEUNet(img_channels=3, out_channels=2, dim=16, depth=5, kernel_size=3, dilation=2, ratio=4, pad=2, shuffle=True, deep_supervision=True, deep_out=5).cuda()
